...
import { CITY_WORLD_SCALE, MAIN_MENU_SPAWN } from "../core/config.js";
...
class RTCVehicle {
  constructor(mesh, waypoints, startIndex) {
    ...
    this.controlWait = 0;
    this.hasStoppedForControl = false;
...
export class TrafficSystem {
  constructor() {
    ...
    this._controlGroup = new THREE.Group();
    this._controlGroup.name = "TrafficControls";
    this._roadMarkingGroup = new THREE.Group();
    this._roadMarkingGroup.name = "RoadCenterLines";
    this._loaded = false;
  ...
  _offsetWaypointsToRightLane(waypoints, fallbackY = 0) {
    for (let i = 0; i < waypoints.length; i++) {
      const prev = waypoints[Math.max(0, i - 1)]?.position;
      const next = waypoints[Math.min(waypoints.length - 1, i + 1)]?.position;
      if (!prev || !next || prev.distanceToSquared(next) < 0.001) continue;

      const dir = _tv[0].subVectors(next, prev);
      dir.y = 0;
  ...
  _addRoadCenterline(waypoints) {
    if (!Array.isArray(waypoints) || waypoints.length < 2) return;
    const points = [];
    for (const wp of waypoints) {
      points.push(wp.position.x, wp.position.y + 0.08, wp.position.z);
    }
    ...
    line.name = "Yellow_Road_Centerline";
    line.frustumCulled = false;
    line.renderOrder = -3;
    this._roadMarkingGroup.add(line);
  }

  _buildTrafficControls() {
    ...
    for (const path of this._waypointPaths) {
      ...
      for (let i = 1; i < path.length - 1; i++) {
        ...
        if (!nearest || nearest === lastControl) continue;
        const prev = path[Math.max(0, i - 1)].position;
        const next = path[Math.min(path.length - 1, i + 1)].position;
        const dir = _tv[0].subVectors(next, prev);
        dir.y = 0;
        wp.control = nearest;
        wp.controlApproach = Math.abs(dir.x) > Math.abs(dir.z) ? "eastwest" : "northsouth";
        wp.radius = Math.max(wp.radius, 4.5);
        lastControl = nearest;
      }
    }

    if (!this._controlGroup.parent) world.root.add(this._controlGroup);
  }

  _createTrafficLight(control) {
    const group = new THREE.Group();
    group.name = "TrafficLight";
    group.position.copy(control.position);
  ...
  _createStopSign(control) {
    ...
    sign.position.set(-5.1, 2.25, -5.1);
  ...
  _approachingControlledWaypoint(vehicle) {
    ...
    const forward = _tv[1].set(0, 0, 1).applyQuaternion(vehicle.mesh.quaternion);
    forward.y = 0;
    if (forward.lengthSq() > 0.001 && toControl.lengthSq() > 0.001 && toControl.normalize().dot(forward.normalize()) < 0.15) {
      return null;
    }
  ...
  _applyTrafficControl(vehicle, dt) {
    const approach = this._approachingControlledWaypoint(vehicle);
    if (!approach) {
      vehicle.stopControl = null;
      vehicle.controlWait = 0;
      vehicle.hasStoppedForControl = false;
      return false;
    }

    const { control, approachPhase, distance } = approach;
    vehicle.stopControl = control;

    if (control.type === "stop") {
      if (vehicle.currentSpeed < SIGNAL_MIN_SPEED && distance < 6) {
        vehicle.controlWait += dt;
      }
      const shouldStop = !vehicle.hasStoppedForControl && vehicle.controlWait < CONTROL_STOP_HOLD;
      if (!shouldStop) vehicle.hasStoppedForControl = true;
      return shouldStop;
    }

    const signalState = this._signalState(control, approachPhase);
    if (signalState === "green") return false;
    if (signalState === "yellow" && (distance < 7 || vehicle.currentSpeed > 32)) return false;
    return true;
  }

  _navigation(vehicle, dt = 1 / 60) {
    ...
    if (this._applyTrafficControl(vehicle, dt)) {
      vehicle.stopNow = true;
